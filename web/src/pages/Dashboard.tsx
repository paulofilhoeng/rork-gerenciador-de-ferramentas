...
import { MOVEMENT_COLOR, MOVEMENT_LABEL, daysRemaining, effectiveStatus, isRentalEndingSoon, totalRentalCost } from "@/lib/types";
...
export default function Dashboard() {
  ...
  const stats = useMemo(() => {
    ...
    return { ownTools, rentedTools, overdueTools, endingSoonTools, inMaintenance, inUseTools, availableTools, activeSites, total };
  }, [db.tools, db.sites]);
  ...
  return (
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button
              type="button"
              disabled={exporting}
              className="flex items-center gap-2 rounded-[10px] bg-app-accent/15 px-3.5 py-2 text-sm font-semibold text-app-accent transition-colors hover:bg-app-accent/25"
            >
              <Download size={15} />
              Exportar
            </button>
          ...
          <DropdownMenuContent align="end" className="border-app-separator bg-app-card text-white">
            ...
            <DropdownMenuItem
              onClick={() => {
                ...
                toast.success("Histórico de movimentações exportado");
              }}
              className="gap-2"
            >
    ...
    >
      <div className="flex flex-col gap-5 pb-6">
        ...
        {/* Alerts */}
        {(stats.overdueTools.length > 0 || stats.endingSoonTools.length > 0 || stats.inMaintenance.length > 0) && (
                  ...
                  <Link key={site.id} to={`/obras/${site.id}`} className="flex items-center gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="truncate text-xs text-app-muted">
                        {toolCount} ferramentas · {site.responsibleName}
                      </p>
                    </div>
